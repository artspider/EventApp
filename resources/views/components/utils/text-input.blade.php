...
<div class="{{$attributes->get('class')}}">
    <label for="{{$attributes->whereStartsWith('wire:model')->first()}}"
        class="block  font-medium leading-5 text-gray-800">{{$label}}</label>
    <div class=" relative rounded-md shadow-md">
        ...
        class="form-input block w-full {{$pl}} {{$pr}} bg-gray-600 border-gray-300 text-main-color
        placeholder-gray-300 focus:ring-indigo-500 focus:border-indigo-500 focus:shadow-outline-red sm:text-sm
        sm:leading-5
        font-bold"
        @else
        class="form-input block w-full {{$pl}} {{$pr}} text-accent-color font-montserrat placeholder-gray-400
        bg-gray-200 font-bold
        border
        border-transparent focus:outline-none focus:ring-2 focus:ring-indigo-400 focus:border-indigo-400
        ...
        <div class="absolute inset-y-0 right-0 pr-3 flex items-center pointer-events-none">
            <svg class="h-5 w-5 fill-current text-red-600" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
                <path
